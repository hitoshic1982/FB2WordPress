...
- macOS／Linux 目前不是完整可發布產品；Avalonia 專案只是能建置、能逐步承接功能的入口。
- CI 成功不得寫成 macOS／Linux 實機驗證。沒有真實裝置、系統安全保管庫與完整搬家流程證據前，不提供正式相容承諾。
...
- macOS／Linux 目前不是完整可发布产品；Avalonia 项目只是能够构建并逐步承接功能的入口。
- CI 成功不能写成 macOS／Linux 实机验证。获得真实设备、系统安全存储和完整迁移流程证据之前，不提供正式兼容承诺。
...
- macOS and Linux are not complete release products. The Avalonia project is only a buildable entry point for incremental migration.
- A successful CI build must not be described as macOS or Linux device validation. No formal compatibility claim will be made before real-device evidence, OS vault integration, and the complete migration workflow exist.
...
- macOS／Linux は現在、完全な公開製品ではありません。Avalonia プロジェクトは段階的な移植のためのビルド可能な入口です。
- CI 成功を macOS／Linux 実機検証と表現しません。実機結果、OS 保管庫、完全な移行フローがそろうまでは正式な互換性を表明しません。
