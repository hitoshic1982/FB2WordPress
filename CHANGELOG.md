...
- 新增真正的 macOS x64 未簽章 Preview DMG 與 Linux x86_64 Preview AppImage；兩者均在對應的原生 GitHub runner 從最終封裝成品執行啟動存活測試。
- Preview 成品附 SHA256 與 SPDX SBOM；非 PR 建置在 GitHub 支援時建立來源與 SBOM 證明。macOS／Linux 仍明確標示為移植入口，尚非完整搬家版或作者實機驗證。
...
- 新增真正的 macOS x64 未签名 Preview DMG 与 Linux x86_64 Preview AppImage；两者均在对应的原生 GitHub runner 中从最终打包成品执行启动存活测试。
- Preview 成品附带 SHA256 和 SPDX SBOM；非 PR 构建在 GitHub 支持时生成来源与 SBOM 证明。macOS／Linux 仍明确标记为移植入口，不是完整迁移版或作者实机验证。
...
- Added a genuine unsigned macOS x64 Preview DMG and Linux x86_64 Preview AppImage. Each final package is launch-smoke-tested on its matching native GitHub runner.
- Preview artifacts include SHA256 and SPDX SBOM evidence; non-PR builds receive provenance and SBOM attestations when GitHub supports them. macOS/Linux remain explicitly incomplete migration entry points without author-owned real-device validation.
...
- 真正な未署名 macOS x64 Preview DMG と Linux x86_64 Preview AppImage を追加し、最終成果物から対応するネイティブ GitHub runner 上で起動・存続テストを行います。
- Preview 成果物に SHA256 と SPDX SBOM を添付し、PR 以外のビルドでは GitHub が対応する場合に来歴・SBOM 証明を生成します。macOS／Linux は未完成の移植入口であり、作者による実機検証済み完全版ではありません。
